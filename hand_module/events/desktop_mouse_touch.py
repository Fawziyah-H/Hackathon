#By Chenuka Ratwatte based off Ali Hassan's work, modified for multitouch and navigating 3d objects.
...
class DesktopMouseTouch:
    def __init__(self, mouse_smoothing=3,FingerRadius=5,MouseSensitivity=1.0):
        ...
        self.touch = Touch(self.finger_radius)
    ...
    def update_offhand_position(self,x,y,is_offhand_clicked):
        """TAKES X,Y INPUTS AND CONVERTS TO Position of offhand relative to screen/active window"""
        
        # upon the offhand click get active window information  
        if self.last_is_offhand_clicked == False and is_offhand_clicked == True:
            try:
                self.active_window_dimensions = GetWindowRect(GetForegroundWindow())
                self.offhand_ref_point = (x,y)
                if self.cursor_raw[0] >= x:
                    window_ref_x = self.active_window_dimensions[0]+(self.cursor.position[0] - self.active_window_dimensions[0])/2
                else:
                    window_ref_x = self.active_window_dimensions[2]-(self.active_window_dimensions[2] - self.cursor.position[0])/2

                self.window_ref_point = (
                    window_ref_x,
                ...
                self.active_window_dimensions = None
                self.window_ref_point = (0,0)
                self.offhand_ref_point = (0,0)

        #get contact position relative to mouse position
        if self.active_window_dimensions is not None:
            rel_position_x = self.window_ref_point[0] + (x-self.offhand_ref_point[0])*self.mouse_sensitivity
            rel_position_y = self.window_ref_point[1] + (y-self.offhand_ref_point[1])*self.mouse_sensitivity
        else:
            rel_position_x = self.cursor.position[0] + (x-self.cursor_raw[0])/2*self.mouse_sensitivity
            rel_position_y = self.cursor.position[1] + (y-self.cursor_raw[1])/2*self.mouse_sensitivity
        
        # prevent buffer from growing too large
        if len(self.offhand_buffer_x) >= self.smoothing + 1:
                self.offhand_buffer_x.pop(0)
                self.offhand_buffer_y.pop(0)
        self.offhand_buffer_x.append(rel_position_x)
        self.offhand_buffer_y.append(rel_position_y)
        self.offhand_xy = [
            int(sum(self.offhand_buffer_x) / len(self.offhand_buffer_x)),
            int(sum(self.offhand_buffer_y) / len(self.offhand_buffer_y))
        ]
    ...
    def click_events(self, is_left_clicked, is_right_clicked, is_double_clicked,is_offhand_clicked, is_panning):
        """This buffering is necessary to avoid jitter in clicking with your offhand"""
        if not is_offhand_clicked and len(self.offhand_buffer_x) > 0:
            self.offhand_buffer_x = []     
            self.offhand_buffer_y = []
        ...
        if is_panning:
            ...
            self.touch.touchdown([self.cursor.position,second_touch])
        ...
        if (is_left_clicked is False and is_double_clicked is False and self.last_is_double_clicked is False and self.last_is_left_clicked is True):
            if self.left_click_counter <= 7:
                self.touch.singletap(self.cursor.position)
                self.left_click_counter = 0
            if self.left_click_counter > 6:
                self.touch.touchup(self.cursor.position)
                self.left_click_counter = 0
            self.drag = False
        ...
        if is_left_clicked is True and is_right_clicked is False and self.left_click_counter > 6:
            if is_offhand_clicked:
                self.touch.touchdown([self.cursor.position,self.offhand_xy])
            else:
                self.touch.touchdown([self.cursor.position])
            self.drag = True
        ...
        self.last_is_offhand_clicked = is_offhand_clicked

    def scroll_events(self, speed=1, direction="Down"):
        speed = (speed * -1 if direction == "Down" else speed)  # downwards scroll gesture = negative scroll value
        self.cursor.scroll(0, speed)

    def update_desktop_click_events(self, is_left_clicked, is_right_clicked, is_double_clicked,is_offhand_clicked, is_panning):
        self.click_events(is_left_clicked, is_right_clicked, is_double_clicked, is_offhand_clicked, is_panning)
